...
class SalesInvoice(CommonController, sales_invoice.SalesInvoice):
	...
	def _get_effective_forma_pago(self) -> str | None:
		"""FormaPago (SAT catalog code) for CFDI. From mx_payment_mode or from Mode of Payment's SAT Payment Method."""
		if getattr(self, "mx_payment_mode", None) and str(self.mx_payment_mode).strip():
			return str(self.mx_payment_mode).strip()
		if getattr(self, "mode_of_payment", None):
	...
	def _get_effective_metodo_pago(self) -> str | None:
		"""MetodoPago (PUE or PPD) for CFDI. From mx_payment_option."""
		return getattr(self, "mx_payment_option", None) or None
