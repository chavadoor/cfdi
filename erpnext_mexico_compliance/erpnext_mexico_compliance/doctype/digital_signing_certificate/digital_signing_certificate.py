...
class DigitalSigningCertificate(Document):
    ...
    def signer(self) -> Signer | None:
        ...
        if self.certificate and self.key and self.certificate == self.key:
            frappe.throw(
                _(
                    "Certificate and Key must be different files. "
                    "Use the .cer file (public certificate) in Certificate and the .key file (private key) in Key."
                ),
                title=_("Invalid Signing Certificate"),
            )
        ...
        try:
            ...
            frappe.throw(msg=str(e), title=_("Invalid Signing Certificate"))
        except crypto.Error as e:
            frappe.throw(
                _(
                    "The Certificate file must be the .cer (public certificate), not the .key file. "
                    "The Key file must be the .key (private key). "
                    "You may have uploaded the same file in both fields."
                ),
                title=_("Invalid Signing Certificate"),
            )
    ...
    def _nombre_emisor_cfdi(self) -> str | None:
        """Nombre del emisor para el CFDI. Si el certificado trunca la razón social (p. ej. termina
        en ' S DE RL D' por límite de 64 caracteres en X.509), se quita ese sufijo para usar la
        razón social corta y evitar CFDI40138."""
        nombre = self.legal_name
        if not nombre:
            return None
        nombre = nombre.strip()
        if nombre.endswith(" S DE RL D"):
            nombre = nombre[: -len(" S DE RL D")].strip()
        return nombre or self.legal_name
    ...
    def get_issuer(self) -> Emisor:
        ...
        return Emisor(
            rfc=self.rfc,
            nombre=self._nombre_emisor_cfdi(),
            regimen_fiscal=company.mx_tax_regime,
        )
