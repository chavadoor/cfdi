...
class CommonController(Document):
	...
	def update_cancellation_status(self):
		...
		Does nothing when the stamping provider is Finkok (status query not supported).
	...
	def check_cancellation_status(self):
		...
		When the stamping provider is Finkok, status query is not supported; a message is shown.
		"""
		settings = frappe.get_single("CFDI Stamping Settings")
		if getattr(settings, "stamping_provider", None) == "Finkok":
			frappe.msgprint(
				_("Status query is not supported for Finkok. Use the SAT verification portal to check CFDI status."),
				title=_("Check Cancellation Status"),
			)
			return None
